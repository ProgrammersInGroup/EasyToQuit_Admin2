      ...
      <div class="vue-panel">
        <Form :model="formItem" ref="formItem" :rules="ruleValidate" :label-width="80">
          ...
          <FormItem label="文章作者:" prop="userCode">
            <Input clearable v-model="formItem.userCode" placeholder="請輸入文章作者"></Input>
          ...
          <FormItem label="文章分類:" prop="category">
            <Input clearable v-model="formItem.category" placeholder="請輸入文章分類"></Input>
          ...
          <FormItem>
            ...
            <Button size="large" type="ghost" class="vue-back-btn" @click="clear" shape="circle" style="margin-left: 8px">清除</Button>
          </FormItem>
        </Form>
      </div>
...
export default {
  ...
  created() {
    ...
    if (this.$route.query && this.$route.query.id) {
      ...
      // 新增用户时提示一下
      this.breadcrumbTitle = "新增衛教文章";
    }
  },
  data() {
    const validateCategory = (rule, value, callback) => {
      if (!this.formItem.userCode) {
        callback(new Error("請輸入文章作者"));
      }
      if (value.length === 0) {
        callback(new Error("請輸入文章分類"));
      } else {
        callback();
      }
    };
